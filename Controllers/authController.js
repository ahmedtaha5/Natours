...
exports.login = CatchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).select('+password'); // (+) because password (select: false) in user model
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) check the token expiration and there is user , set the password
  if (!user) {
